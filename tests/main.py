...
from src.rppg_dataset_loaders import loader_dccsfedu, loader_ubfc, loader_mahnob, \
    loader_deap, loader_viplhr, DSTitle
from src.rppg_dataset_loaders.loader_base import TimestampAlignment
...
def main(dir_datasets: str):
    pp = pprint.PrettyPrinter(indent=2)

    path_datasets = pathlib.Path(dir_datasets)
    if not path_datasets.exists():
        raise FileNotFoundError(f'Wrong path for `dir_datasets`: {path_datasets}')

    loader = loader_dccsfedu.DCCSFEDUDatasetLoader(path_datasets / DSTitle.DCC_SFEDU.value)
    ...
    loader = loader_ubfc.UBFCDatasetLoader(path_datasets / DSTitle.UBFC.value / 'DATASET_2')
    ...
    # RePSS datasets are not used after the competition
    # loader = loader_repss_train.REPSS_TRAINDatasetLoader(path_datasets / DSTitle.RePSS_Train.value)
    ...
    #
    # loader = loader_repss_test.REPSS_TESTDatasetLoader(path_datasets / DSTitle.RePSS_Test.value)
    # test_loader(loader)
    ...
    loader = loader_mahnob.MahnobDatasetLoader(path_datasets / DSTitle.Mahnob.value)
    ...
    loader = loader_deap.DEAPDatasetLoader(path_datasets / DSTitle.DEAP.value)
    test_loader(loader)
    s1 = loader.get_session_by_index(1)
    ...
    loader = loader_viplhr.VIPLHRDatasetLoader(path_datasets / DSTitle.VIPL.value)
    test_loader(loader)
    ...
    # v = MahnobLoader.MahnobVideoChannel(
    #     "vidos", "MahnobHCI\\Sessions\\36\\P1-Rec1-2009.07.09.17.53.46_C1 trigger _C_Section_36.avi"
    # )
    # pp.pprint(v._get_frame_timestamps())


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Tests for rppg-datasets')
    parser.add_argument('--dir_datasets', type=str, default='datasets', metavar='path_to_datasets_folder',
                        help='path to directory containing datasets')
    args = parser.parse_args()
    main(dir_datasets=args.dir_datasets)
